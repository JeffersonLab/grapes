 ...
 */
public class NeutronProtonFDWagon extends BeamTargetWagon {
    ...
    public NeutronProtonFDWagon() {
        super("NeutronProtonFDWagon","Bobcat","0.0");
    }
	...
	boolean fd_proton = false;
	boolean fd_neutron = false; 
	int proton_mult = 0;

        for (int i=1; i<particles.getRows(); ++i) {
            if (particles.getInt("pid",i) == 2212) {
		proton_mult++;
		if (particles.getInt("status",i)%2000 < 1000)
		    fd_proton = true;
	    }

            if (particles.getInt("pid",i) == 2112) {
		if (particles.getInt("status",i)%2000 < 1000)
		    fd_neutron = true;
	    }
	...
	if(proton_mult >= 2 && fd_proton)
	    return true;
	else if(proton_mult >= 1 && fd_neutron)
	    return true;
	else
	    return false;
    }
